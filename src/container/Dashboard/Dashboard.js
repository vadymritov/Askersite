...
const Dashboard = (props) => {
  ...
  useEffect(() => {
    if (typeTab === "active") {
      http
        .post("activeAsker", `user_id=${localStorage.getItem("UserID")}`)
        .then((resp) => resp.data)
      ...
      http
        .post("allAsker", `user_id=${localStorage.getItem("UserID")}`)
        .then((resp) => resp.data)
        .then((askersData) => setArrAsker(askersData.asker));
    }
  }, [typeTab]);
  ...
  return (
    <div className={styles.dashboardContainer}>
      <div
        ref={cardRef}
        className={`default-flip flip-card-inner  ${styles.createContainer}`}
      >
